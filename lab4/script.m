...
fprintf('---------------------------------------------------------------------\n');
fprintf('j   |L3(x_j)-f(x_j)|   |P3(x_j)-f(x_j)|   Разность погрешностей\n');
fprintf('---------------------------------------------------------------------\n');
for j = 1:length(x_j)
    diff_err = abs(errors_L(j) - errors_P(j));
    fprintf('%d   %16.6e   %16.6e   %16.2e\n', ...
            j-1, errors_L(j), errors_P(j), diff_err);
end
fprintf('---------------------------------------------------------------------\n');

if max(abs(errors_L - errors_P)) < 1e-12
    fprintf('\nВывод: погрешности для многочленов Лагранжа и Ньютона совпадают\n');
    fprintf('       с точностью до машинной точности — это один и тот же полином.\n');
else
    fprintf('\nВнимание: обнаружено расхождение между погрешностями!\n');
end
