...
disp(x_A);
fprintf('Решение системы A1x = b1:\n');
disp(x_A1);
...
% Вычисляем норму матрицы B и критерий остановки
B_norm = norm(B, 1);
...
diff_norm = stopping_criterion_value + 1; % Гарантирует вход в цикл while

fprintf('┌─────┬────────────┬────────────┬────────────┬────────────┬──────────────────┬────────────────────┐\n');
fprintf('│ №   │    x1      │    x2      │    x3      │    x4      │ ‖x^(k)-x^(k-1)‖  │ ((1-||B||₁)/||B||₁)*ε     │\n');
fprintf('├─────┼────────────┼────────────┼────────────┼────────────┼──────────────────┼────────────────────┤\n');

while diff_norm > stopping_criterion_value
    k = k + 1;
    x_new = B * x_old + c;
    diff_norm = norm(x_new - x_old, 1);

    fprintf('│ %3d │ %10.6f │ %10.6f │ %10.6f │ %10.6f │ %16.6e │ %18.6e │\n', ...
            k, x_new(1), x_new(2), x_new(3), x_new(4), diff_norm, stopping_criterion_value);
